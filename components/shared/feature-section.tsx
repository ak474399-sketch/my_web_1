    ...
    <section className="border-t border-warm-200 bg-warm-100">
      <div className="container mx-auto px-4 py-12 sm:py-20 md:py-28">
        <div className="text-center mb-10 sm:mb-16">
          ...
          <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl font-bold text-warm-800">{t("feature.restoreWhatMatters")}</h2>
        </div>

        <div className="max-w-6xl mx-auto space-y-14 sm:space-y-24">
          {FEATURE_KEYS.map((key, i) => {
            ...
            return (
              <motion.div
                ...
                className={`flex flex-col ${reversed ? "md:flex-row-reverse" : "md:flex-row"} items-center gap-6 sm:gap-10 md:gap-16`}
              >
                <div className="flex-1 space-y-5">
                  <span className="inline-block text-sm font-medium text-accent bg-accent/10 rounded-full px-4 py-1">{t(`${prefix}.tag`)}</span>
                  <h3 className="font-serif text-xl sm:text-2xl md:text-3xl font-bold text-warm-800 leading-tight">{t(`${prefix}.title`)}</h3>
